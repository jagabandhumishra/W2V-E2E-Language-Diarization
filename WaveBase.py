...
# python WaveBase.py --savedir " " \
# --train " " \
# --test " " \
# --seed 0 --device 0 --batch 32 --epochs 60 --lang 3 --model my_atten_model --lr 0.0001 --maxlength 666 --lmbda 0.5 --fll ""
...
def main():
    ...
    #load model
    model = X_Base_E2E_LD(n_lang=args.lang,
                                dropout=0.1,
                                feat_dim=256,
                                n_heads=4,
                                d_k=256,
    ...
    loss_func_CRE = nn.CrossEntropyLoss(weight=class_weights,reduction='mean').to(device)
    loss_func_xv = nn.CrossEntropyLoss(weight=class_weights,reduction='mean',ignore_index=255).to(device) # this is important since 255 is for zero paddings
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        pd.DataFrame(data={"Epoch": Epoch, "Curr_LR":Curr_LR, "Curr_Acc": Curr_Acc,"LOSS":Loss,"EER":EER,'eer0':eer0,'eer1':eer1,'eer2':eer2, 'N_Total':N_TOTAL,'N_L1':N_L1, 'N_L2':N_L2, 'N_S':N_S, 'ACCURACY':ACCURACY, 'RECL_L1':RECL_L1,'RECL_L2':RECL_L2,'RECL_S':RECL_S,'CMP':CMP, 'CMA':CMA}).to_csv(args.savedir+"/Model_Details_wavvec.csv")
